...
import markerStylers from '../maps/marker-stylers.js';
...
var infoWindowObj = {
  ...
  populateInfoWindow: function(map, infoWindow,
    ...
    var videoWidth = infoWindowObj.width;

    // Determine optimal thumb size
    var thumbWidth = infoWindowObj.getThumbWidth();
  ...
  ViewModel: function() {
    ...
    self.fetchVideo = function(thumb) {
      infoWindowObj.embedVideo(thumb.link, self.embeddedVideo);
    };

    // Add links to videos and embed the top result.
    infoWindowObj.renderVideoContent(self.videoThumbs, self.embeddedVideo);
  },
  ...
  addFromQuery: function(searchTerms, thumbsKO, embeddedVideoKO) {
    ...
    $.ajax({
      ...
      if(dataAry.length > 0) {
        if(embeddedVideoKO() === '') {
          infoWindowObj.embedVideo(dataAry[0].link, embeddedVideoKO);
        }
      ...
      if(searchTerms.length > 0) {
        ...
        infoWindowObj.addFromQuery(searchTerms, thumbsKO, embeddedVideoKO);
      }
    }).fail(function(err) {
      embeddedVideoKO(infoWindowObj.vimeoErrMsg);
    });
  },
